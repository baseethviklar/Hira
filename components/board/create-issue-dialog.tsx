...
const formSchema = z.object({
    ...
    originalEstimate: z.string().optional(), // User inputs string "2h"
}).refine((data) => {
...
export function CreateIssueDialog({ projectId, statuses }: CreateIssueDialogProps) {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        setIsLoading(true);
        try {
            const estimateMinutes = values.originalEstimate ? parseDuration(values.originalEstimate) : 0;

            await createIssue({
                ...values,
                projectId,
                originalEstimate: estimateMinutes > 0 ? estimateMinutes : undefined
            });
